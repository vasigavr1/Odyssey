  ...
  if (ENABLE_ASSERTIONS) {
    ...
    if (message_l_id > MAX_W_COALESCE && info->valid_header_l_id) {
      ...
      if ((prev_mes_valid_l_id) && (message_l_id != prev_l_id + prev_write_num)) {
        red_printf("Worker %u: Current message l_id %u, previous message l_id %u , prev_write_num %u\n",
                   t_id, message_l_id, prev_l_id, prev_write_num);
      }
    }
    // Check the versions
                                        ...
                                        uint16_t w_num, uint8_t mes_opcode, uint16_t t_id)
{
  if (ENABLE_ASSERTIONS) {
    ...
    if (mes_opcode == ONLY_WRITES) assert(write->opcode != ACCEPT_OP);
    else if (mes_opcode == ONLY_ACCEPTS) assert(write->opcode == ACCEPT_OP);
...
static inline void print_polled_write_message_info(struct w_message *w_mes, uint32_t index, uint16_t t_id)
{
  if (ENABLE_ASSERTIONS) {
    if (DEBUG_WRITES && w_mes->write[0].opcode != ACCEPT_OP)
      ...
      struct commit *com = (struct commit *) &w_mes->write[0];
...
static inline void on_starting_an_acquire_query_the_conf(const uint16_t t_id, struct read_info *r_info)
{
  if (unlikely(conf_bit_vec[machine_id].bit == DOWN_STABLE)) {
    increment_epoch_id(r_info->epoch_id, t_id);
   ...
   uint32_t backward_ptr, uint16_t t_id)
{
  ...
  uint8_t failed_machine_num = 0 ;//q_info->missing_num;
...
static inline uint16_t get_write_size_from_opcode(uint8_t opcode) {
  check_state_with_allowed_flags(10, opcode, OP_RELEASE, CACHE_OP_PUT, ACCEPT_OP,
                               ...
                               uint8_t source, uint16_t t_id)
{
  ...
  if (source == FROM_TRACE) {
      ...
      // Query the conf to see if the machine has lost messages
      if (r_info->opcode == OP_ACQUIRE) on_starting_an_acquire_query_the_conf(t_id, r_info);
  }
                              ...
                              uint16_t session_id, uint16_t t_id)
{
  ...
  bool new_message_because_of_accepts = is_accept && w_mes->coalesce_num > 0 &&
                                                     w_mes->opcode != ONLY_WRITES;
                                ...
                                uint8_t rem_m_id, bool coalesce,  uint8_t read_opcode)
{
 check_state_with_allowed_flags(2, read_opcode, OP_ACQUIRE_FLIP_BIT);
  ...
  r_rep->opcode = TS_EQUAL;
  finish_r_rep_bookkeeping(p_ops, r_rep, 0, rem_m_id, t_id);
}
  ...
  while (polled_messages < completed_messages) {
    ...
    for (uint16_t i = 0; i < r_num; i++) {
      ...
      if (is_propose) {
        ...
        p_ops->ptrs_to_mes_ops[polled_reads] = (((void *) prop) -3); //TODO align with the cache op
      }
      else {
  ...
  if (w_meta->w_state >= SENT_PUT && w_meta->acks_seen >= REMOTE_QUORUM) {
    ...
    if (p_ops->full_w_q_fifo == WRITE_FIFO_TIMEOUT) {
      ...
      for (uint32_t i = 0; i < p_ops->w_size; i++) {
        ...
        if (w_meta->w_state >= SENT_PUT && w_meta->acks_seen >= REMOTE_QUORUM) {
//          bool update_sess_info = false;
//          switch(w_meta->w_state) {
//            case SENT_PUT:
//              w_meta->w_state = READY_PUT;
//              update_sess_info = true;
//              break;
//            case SENT_RELEASE:
//              w_meta->w_state = READY_RELEASE;
//              update_sess_info = true;
//              break;
//            case SENT_COMMIT:
//              update_sess_info = true;
//              act_on_quorum_of_commit_acks(p_ops, w_pull_ptr, t_id);
//              break;
//            case SENT_ACQUIRE:
...
//              break;
//            default:
//              if (w_meta->w_state >= READY_PUT && w_meta->w_state <= READY_NO_OP_RELEASE)
//                break;
//              red_printf("Wrkr %u state %u, ptr %u \n", t_id, w_meta->w_state, w_pull_ptr);
//              assert(false);
//          }
//
//
                                       ...
                                       uint16_t op_i, uint16_t t_id)
{
  ...
  finish_r_rep_bookkeeping(p_ops, (struct r_rep_big*) acc_rep, 0, acc_m_id, t_id);
                                                             ...
                                                             uint16_t t_id)
{
  ...
  finish_r_rep_bookkeeping(p_ops, r_rep, op->opcode, rem_m_id, t_id);
                                      ...
                                      uint16_t t_id)
{
  ...
  finish_r_rep_bookkeeping(p_ops, (struct r_rep_big*) prop_rep, 0, prop_m_id, t_id);
                                      ...
                                      uint16_t t_id)
{
  ...
  finish_r_rep_bookkeeping(p_ops, (struct r_rep_big *) acq_rep, 0, rem_m_id, t_id);
}
