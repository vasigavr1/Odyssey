                                  ...
                                  uint16_t t_id, loc_entry_t* loc_entry)
{
  ...
  if (ENABLE_ASSERTIONS)
    assert(loc_entry->rmw_id.glob_sess_id == loc_entry->glob_sess_id);
                                    ...
                                    uint32_t log_no, uint16_t t_id, const char *message)
{
  ...
  kv_ptr->rmw_id.glob_sess_id = rmw_id % GLOBAL_SESSION_NUM;
...
static inline void take_actions_to_commit_rmw(loc_entry_t *loc_entry_to_commit,
...
static inline void attempt_local_commit(p_ops_t *p_ops, loc_entry_t *loc_entry,
                                        uint16_t t_id)
{
                                                 ...
                                                 loc_entry_t* loc_entry, uint16_t t_id)
{
  ...
  if (kv_ptr->last_committed_log_no < new_log_no) {
    ...
    kv_ptr->last_committed_rmw_id.glob_sess_id = (uint16_t ) (new_rmw_id % GLOBAL_SESSION_NUM);
    ...
    if (DEBUG_RMW)
      my_printf(green, "Wrkr %u commits locally rmw id %u, after resp with opcode %u \n",
                t_id, new_rmw_id, rmw_rep->opcode);
    ...
    if (kv_ptr->log_no <= new_log_no || kv_ptr->rmw_id.id == new_rmw_id) {
      kv_ptr->log_no = new_log_no;
      kv_ptr->state = INVALID_RMW;
    }
  }
  else if (kv_ptr->last_committed_log_no == new_log_no) {
    check_that_the_rmw_ids_match(kv_ptr,  new_rmw_id, new_log_no, rmw_rep->ts.version,
  ...
  register_committed_global_sess_id(new_rmw_id, t_id);
  check_registered_against_kv_ptr_last_committed(kv_ptr, new_rmw_id,
                                                 "attempt_local_commit_from_rep", t_id);
  unlock_seqlock(&kv_ptr->seqlock);


  if (DEBUG_LOG)
    my_printf(green, "Log %u: RMW_id %u loc entry state %u from reply \n",
              new_log_no, new_rmw_id, loc_entry->state);
...
static inline bool attempt_remote_commit_no_value(mica_op_t *kv_ptr, struct commit_no_val *com,
                                                  uint16_t t_id)
{
  uint32_t new_log_no = com->log_no;
  ...
  if (kv_ptr->last_committed_log_no < new_log_no) {
    if (ENABLE_ASSERTIONS) {
      assert(kv_ptr->state == ACCEPTED);
      assert(kv_ptr->log_no == com->log_no);
      assert(kv_ptr->accepted_rmw_id.id == com->t_rmw_id);
    }
    is_log_higher = true;
    kv_ptr->last_committed_log_no = new_log_no;
    kv_ptr->last_committed_rmw_id.id = com->t_rmw_id;
    kv_ptr->last_committed_rmw_id.glob_sess_id = (uint16_t) (com->t_rmw_id % GLOBAL_SESSION_NUM);
    kv_ptr->state = INVALID_RMW;
    if (DEBUG_LOG)
      my_printf(green, "Log %u: RMW_id %u glob_sess %u ,from remote_commit \n",
                new_log_no, com->t_rmw_id, (uint16_t) com->t_rmw_id % GLOBAL_SESSION_NUM);
  }
  else if (kv_ptr->last_committed_log_no == new_log_no) {
    check_that_the_rmw_ids_match(kv_ptr, com->t_rmw_id, new_log_no,
                                 kv_ptr->base_acc_ts.version, kv_ptr->base_acc_ts.m_id, "attempt_remote_commit without val", t_id);
  }
...
static inline bool attempt_remote_commit(mica_op_t *kv_ptr, struct commit *com,
                                         r_info_t *r_info, bool use_commit,
                                         uint16_t t_id)
{
  uint32_t new_log_no = use_commit ? com->log_no : r_info->log_no;
  uint64_t new_rmw_id = use_commit ? com->t_rmw_id : r_info->rmw_id.id;
  uint16_t glob_sess_id = use_commit ? (uint16_t) (com->t_rmw_id % GLOBAL_SESSION_NUM) : r_info->rmw_id.glob_sess_id;
                                                   ...
                                                   struct rmw_rep_last_committed* prop_rep, uint16_t t_id)
{
  ...
  if (help_loc_entry->state == INVALID_RMW ||
    ...
    help_loc_entry->rmw_id.glob_sess_id = (uint16_t) (prop_rep->rmw_id % GLOBAL_SESSION_NUM);
                                                   ...
                                                   uint16_t t_id)
{
  ...
  if (loc_entry->state == RETRY_WITH_BIGGER_TS) {
    ...
    take_kv_ptr_with_higher_TS(p_ops, loc_entry, (new_version + 1), true, t_id);
                                                    ...
                                                    uint16_t t_id)
{
  ...
  if (loc_entry->state == RETRY_WITH_BIGGER_TS) {
    ...
    take_kv_ptr_with_higher_TS(p_ops, loc_entry, (new_version + 1), false, t_id);
    check_state_with_allowed_flags(5, (int) loc_entry->state, INVALID_RMW, PROPOSED,
                                   NEEDS_KV_PTR, MUST_BCAST_COMMITS);
    if (loc_entry->state == PROPOSED) {
      insert_prop_to_read_fifo(p_ops, loc_entry, t_id);
                                    ...
                                    uint16_t t_id)
{
  uint32_t new_version = 0;
  loc_entry_t tmp;
  loc_entry_t *dbg_loc_entry = &tmp;
  bool zero_out_log_too_high_cntr = true;
  if (ENABLE_ASSERTIONS) memcpy(dbg_loc_entry, loc_entry, sizeof(loc_entry_t));
  ...
  // SEEN HIGHER-TS PROPOSE OR ACCEPT
    ...
    loc_entry->state = RETRY_WITH_BIGGER_TS;
    new_version = rep_info->seen_higher_prop_version;
  ...
  clean_up_after_inspecting_props(p_ops, loc_entry, new_version, dbg_loc_entry, zero_out_log_too_high_cntr, t_id);
}
                                   ...
                                   uint16_t t_id)
{
  ...
  loc_entry_t *dbg_loc_entry = &tmp;
  if (ENABLE_ASSERTIONS)
    memcpy(dbg_loc_entry, loc_entry, sizeof(loc_entry_t));
  ...
  // SEEN HIGHER-TS PROPOSE
    ...
    loc_entry->state = RETRY_WITH_BIGGER_TS;
    new_version = rep_info->seen_higher_prop_version;
    check_version(new_version, "inspect_accepts: rep_info->seen_higher_prop > 0");
    if (ENABLE_ASSERTIONS) assert(loc_entry->helping_flag == NOT_HELPING);
  }
    // LOG TOO HIGH
  else if (rep_info->log_too_high > 0) {
