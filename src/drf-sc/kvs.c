...
void cache_init(int cache_id, int num_threads) {
	...
	KVS.num_threads = num_threads;
//	cache_reset_total_ops_issued();
	/// allocate and init metadata for the cache & threads
//	extended_cache_meta_reset(&KVS.aggregated_meta);
	KVS.meta = malloc(num_threads * sizeof(struct cache_meta_stats));
//	for(i = 0; i < num_threads; i++)
//		cache_meta_reset(&KVS.meta[i]);
	mica_init(&KVS.hash_table, cache_id, KVS_SOCKET, CACHE_NUM_BKTS, HERD_LOG_CAP);
	cache_populate_fixed_len(&KVS.hash_table, CACHE_NUM_KEYS, VALUE_SIZE);
}
...
//		op.key.meta.state = VALID_STATE;
  ...
  // 	   KVS.hash_table.instance_id, n, val_len,
  // 	   (double) KVS.hash_table.num_index_evictions / KVS.hash_table.num_insert_op);
}
  ...
  uint32_t r_push_ptr = p_ops->r_push_ptr;
	for(op_i = 0; op_i < op_num; op_i++) {
        ...
        if (kv_ptr[op_i]->opcode == KEY_IS_NOT_RMWABLE) {
          ...
          else if (op[op_i].opcode == CACHE_OP_GET) {
...
inline void cache_isolated_op(int t_id, struct write *write)
{
  ...
  //printf("op bkt %u\n", op->key.bkt);
  bkt = op->key.bkt & KVS.hash_table.bkt_mask;
  bkt_ptr = &KVS.hash_table.ht_index[bkt];
  ...
  for(j = 0; j < 8; j++) {
    if(bkt_ptr->slots[j].in_use == 1 &&
      ...
      /*
               ...
               */
      kv_ptr = (struct cache_op *) &KVS.hash_table.ht_log[log_offset];
      /* Detect if the head has wrapped around for this index entry */
      if(KVS.hash_table.log_head - bkt_ptr->slots[j].offset >= KVS.hash_table.log_cap) {
...
/*
 * WARNING: the following functions related to kvs stats are not tested on this version of code
 */
...
//	KVS.total_ops_issued = 0;
//}
//
//void cache_meta_aggregate(){
//	int i = 0;
//	for(i = 0; i < kvs.num_threads; i++){
//		kvs.aggregated_meta.metadata.num_get_success += kvs.meta[i].num_get_success;
//		kvs.aggregated_meta.metadata.num_put_success += kvs.meta[i].num_put_success;
//		kvs.aggregated_meta.metadata.num_upd_success += kvs.meta[i].num_upd_success;
//		kvs.aggregated_meta.metadata.num_inv_success += kvs.meta[i].num_inv_success;
//		kvs.aggregated_meta.metadata.num_ack_success += kvs.meta[i].num_ack_success;
//		kvs.aggregated_meta.metadata.num_get_stall += kvs.meta[i].num_get_stall;
//		kvs.aggregated_meta.metadata.num_put_stall += kvs.meta[i].num_put_stall;
//		kvs.aggregated_meta.metadata.num_upd_fail += kvs.meta[i].num_upd_fail;
//		kvs.aggregated_meta.metadata.num_inv_fail += kvs.meta[i].num_inv_fail;
//		kvs.aggregated_meta.metadata.num_ack_fail += kvs.meta[i].num_ack_fail;
//		kvs.aggregated_meta.metadata.num_get_miss += kvs.meta[i].num_get_miss;
//		kvs.aggregated_meta.metadata.num_put_miss += kvs.meta[i].num_put_miss;
//		kvs.aggregated_meta.metadata.num_unserved_get_miss += kvs.meta[i].num_unserved_get_miss;
//		kvs.aggregated_meta.metadata.num_unserved_put_miss += kvs.meta[i].num_unserved_put_miss;
//	}
//	kvs.aggregated_meta.num_hit = kvs.aggregated_meta.metadata.num_get_success + kvs.aggregated_meta.metadata.num_put_success;
//	kvs.aggregated_meta.num_miss = kvs.aggregated_meta.metadata.num_get_miss + kvs.aggregated_meta.metadata.num_put_miss;
//	kvs.aggregated_meta.num_stall = kvs.aggregated_meta.metadata.num_get_stall + kvs.aggregated_meta.metadata.num_put_stall;
//	kvs.aggregated_meta.num_coherence_fail = kvs.aggregated_meta.metadata.num_upd_fail + kvs.aggregated_meta.metadata.num_inv_fail
//											   + kvs.aggregated_meta.metadata.num_ack_fail;
//	kvs.aggregated_meta.num_coherence_success = kvs.aggregated_meta.metadata.num_upd_success + kvs.aggregated_meta.metadata.num_inv_success
//												  + kvs.aggregated_meta.metadata.num_ack_success;
//}
//
...
//	struct extended_cache_meta_stats* meta = &kvs.aggregated_meta;
...
//	if(total_ops != kvs.total_ops_issued){
//		printf("Total_ops: %llu, 2nd total ops: %llu\n",total_ops, kvs.total_ops_issued);
//	}
//	//assert(total_ops == kvs.total_ops_issued);
//	print_IOPS_and_time(start, kvs.total_ops_issued, cache_id);
